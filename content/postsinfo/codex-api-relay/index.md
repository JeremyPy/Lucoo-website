...
<p class="lucoo-token-warning-block">重点：创建令牌时必须配置 token 分组，选择 Plus 号池、Pro 号池或你实际开通的可用分组。没有配置 token 分组，后面 Codex 可能认证成功但无法正常请求模型。</p>
...
| API Key | 你在中转站复制的 `sk-` 开头令牌，且该令牌必须已配置 token 分组 |
| API 请求地址 | 国内默认 <span class="lucoo-red-url">https://api.lucoo.net/v1</span>，海外用户可用 <span class="lucoo-red-url">https://apicc.lucoo.net/v1</span> |
| 模型名称 | 按你开通的模型填写，示例为 `gpt-5.5` |

<p class="lucoo-token-warning-block">填写 API Key 前再次确认：token 分组必须配置。没有分组或分组不匹配时，CC Switch/Codex 里的配置看起来可能正常，但实际调用会失败。</p>
...
2. <span class="lucoo-token-warning">token 分组必须配置</span>，并且分组要有你填写模型的调用权限。
3. 国内地址使用 `https://api.lucoo.net/v1`，海外代理访问使用 `https://apicc.lucoo.net/v1`。
4. 修改配置后是否完全重启 Codex 或 VS Code。
...
- <span class="lucoo-token-warning">token 分组必须配置</span>，否则 API Key 很容易出现保存成功但不可用。
