...
![选择对应版本下载](content/postsinfo/cockpit-local-reverse-proxy/1778770123061.png)

### 2. 完成安装

下载后按系统提示完成安装。安装完成后打开 Cockpit。

![完成安装](content/postsinfo/cockpit-local-reverse-proxy/1778770142841.png)

### 3. 进入首页

打开后可以看到多个工具分类。本文主要以 Codex 为例演示本地反代和多账号配置。

![Cockpit 首页](content/postsinfo/cockpit-local-reverse-proxy/1778770298105.png)
...
![点击 Codex](content/postsinfo/cockpit-local-reverse-proxy/1778770336365.png)

### 2. 选择 API Key 方式

进入 Codex 页面后，点击「API Key」。

![点击 API Key](content/postsinfo/cockpit-local-reverse-proxy/1778770358510.png)
...
![选择 Custom 并填写 API Key](content/postsinfo/cockpit-local-reverse-proxy/1778770465564.png)
...
![新增 Lucoo 模块](content/postsinfo/cockpit-local-reverse-proxy/1778770579599.png)

### 5. 启动并写入 Codex 配置

点击账号卡片上的开始按钮。Cockpit 会自动弹出 Codex 客户端，并完成本地配置。

![启动 Codex 客户端](content/postsinfo/cockpit-local-reverse-proxy/1778770626262.png)

### 6. 测试是否可用

进入 Codex 后发起一次简单测试。如果页面能正常响应、链接正常打开，说明 Lucoo API Key 已经接入成功。

![Codex 测试正常](content/postsinfo/cockpit-local-reverse-proxy/1778770649206.png)
...
![新增 ChatGPT 账号入口](content/postsinfo/cockpit-local-reverse-proxy/1778770736220.png)

### 2. 在浏览器打开登录

点击「在浏览器打开」。

![在浏览器打开](content/postsinfo/cockpit-local-reverse-proxy/1778770807180.png)

### 3. 登录 ChatGPT

这一步和正常登录 ChatGPT 一样，可以使用账号密码、邮箱验证码等方式完成登录。如果没有账号，可以通过店铺地址购买后再登录。

![登录 ChatGPT](content/postsinfo/cockpit-local-reverse-proxy/1778770875832.png)

### 4. 等待授权成功

登录完成后，浏览器页面会出现授权成功提示。

![授权成功](content/postsinfo/cockpit-local-reverse-proxy/1778770895452.png)

### 5. 返回 Cockpit

回到 Cockpit 后，可以看到账号已经登录成功。

![Cockpit 登录提示](content/postsinfo/cockpit-local-reverse-proxy/1778770917336.png)
...
![账号切换并修复会话](content/postsinfo/cockpit-local-reverse-proxy/1778771039173.png)

打开 Codex 后，可以看到已经切换到刚刚登录的账号。

![Codex 切换到 ChatGPT 账号](content/postsinfo/cockpit-local-reverse-proxy/1778771079766.png)
...
![选择账号并保存集合](content/postsinfo/cockpit-local-reverse-proxy/1778771177295.png)
...
![继续启动](content/postsinfo/cockpit-local-reverse-proxy/1778771324260.png)

Cockpit 会继续修复会话并完成 Codex 启动。

![完成 Codex 启动](content/postsinfo/cockpit-local-reverse-proxy/1778771348169.png)

### 5. 确认本地服务名称

启动成功后，服务名称会显示为 `codex api service`。

![codex api service](content/postsinfo/cockpit-local-reverse-proxy/1778771383462.png)

### 6. 再次测试

在 Codex 中发起一次测试，确认可以正常使用。

![集合测试正常](content/postsinfo/cockpit-local-reverse-proxy/1778771400058.png)

### 7. 查看额度

回到 Cockpit 查看额度情况。如果额度正常变化，说明请求已经走到了当前账号或当前渠道。

![查看额度情况](content/postsinfo/cockpit-local-reverse-proxy/1778771485833.png)
...
![会话管理](content/postsinfo/cockpit-local-reverse-proxy/1778771626640.png)

### 2. 唤醒任务

如果有定时任务需要 AI 处理，可以使用唤醒任务。这里可以自定义提示词，让工具在指定场景自动唤醒。

![唤醒任务](content/postsinfo/cockpit-local-reverse-proxy/1778771726960.png)

### 3. 兼容其他平台

Cockpit 还兼容其他平台，例如 Google 相关工具和其他 AI 编程工具。

![兼容其他平台](content/postsinfo/cockpit-local-reverse-proxy/1778771761488.png)

### 4. 导出账号配置

如果账号需要给别人使用，也可以通过导出功能分享配置。导出前注意确认里面是否包含敏感信息。

![导出账号配置](content/postsinfo/cockpit-local-reverse-proxy/1778771840706.png)
